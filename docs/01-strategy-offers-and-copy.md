# Beekal — Strategy, Offer Architecture and Copy System

How the 42-book library becomes concrete decisions about what Beekal sells, how
it is priced, and what words appear on the screen.

---

## 1. Which books actually matter here

The library has 42 titles. Sorted by what they can change about this website:
...
`$100M Hook` and `$100M GOATed Ads` (headline and hook structure), `$100M Closing`
and `The 9 Figure Sales Script` (CTA and call flow), `Talk Less Sell More` (cut
every sentence that is not doing work), `People Buy from People` (founder-led
trust), `$100M Branding` (repeated naming), `$100M Lead Nurture` and
`$100M Marketing Machine` (the email sequences behind the forms).

**Tier 3 — shapes how Beekal is run**

`The Personal MBA` (value creation, marketing, sales, value delivery, finance as
five interlocking systems), `Customer Management`, `Market Research`,
`Business Mindset`, `Million Dollar Weekends` (ship the offer before the
infrastructure), `The War of Art` and `Big Magic` (the founder's own shipping
discipline), `How to Win Friends and Influence People` (the tone of the
founder-reply email).

**Explicitly not applied:** `Clothing Business Guide`, `Seasonal Business Guide`,
`Meta Andromeda`, `Checkout Strategy`, `Elon Musk`, `Shoe Dog`, `Steal Like an
Artist`. Wrong business model or biography; forcing them in would produce noise.
...
> prompt disagree, the master prompt wins.

---

## 2. Positioning (Product Positioning, Blue Ocean)

### Competitive alternatives — what the buyer does if Beekal does not exist

1. Hire another software agency, who quotes the build they were asked for
2. Buy another SaaS product, adding a seventh disconnected system
3. Hire in-house developers, who inherit the same undefined problem
4. Assign it to the IT manager, who has no mandate to redesign processes
5. **Do nothing** — the most common competitor, and the one the copy must beat
...
> working as one system, **Beekal** is the business technology transformation
> partner that **diagnoses the operation before writing code** — unlike software
> agencies, who sell build capacity for a problem nobody has defined yet.

### The line that must survive every rewrite
...
weakest dimensions and where to look first, with nothing withheld. The paid
Assessment is not the same thing at higher resolution — it is the difference
between a self-assessment and an audit, which is a difference the buyer can feel.

The PDF report is the ask, and it is a fair trade because it is genuinely more
than the on-screen result: their radar chart, a page per weak dimension, and the
questions to ask their own team.

### 3.2 Core offer — Business System Assessment

Run through the `$100M Offers` **Value Equation**:
...
**Pricing** (`$100M Pricing`). Fixed price, agreed in writing before the work
starts, never hourly — hourly billing prices our efficiency as a penalty and
invites scope arguments. Anchor against the cost of the wrong build, not against
consulting day rates: the Assessment costs a fraction of one mis-specified
project. Publish a **range** rather than a single number so the buyer can
self-qualify without a call. Beekal is free to raise it as case studies accumulate
(`$100M Price Raise`); the site must never hard-code a price, which is why price
is an admin field.
...
### 3.3 Upsell — Build / Modernize / Automate / AI
...
### 3.4 Continuity — Beekal Care

`$100M Retention` and `$100M Lifetime Value` make the arithmetic plain: it is the
retainer, not the project, that makes the business survivable. Acquisition for a
project is expensive and non-recurring; Care converts each win into an annuity and
funds the productization stage.

Positioned as **continuous technology improvement**, never maintenance —
maintenance is a cost line a CFO cuts, improvement is a budget line a COO defends.
Monthly, with a visible artefact: a monthly technology review the client receives
whether or not anything broke. The artefact is what makes the invoice obvious.

---

## 4. Proof, when there is not much yet
...
3. **Founder specificity.** `People Buy from People`: a named person, a real
   ...
   real photo.
4. **The free tool working properly.** A maturity score that gives a sharp,
   uncomfortable, accurate answer is a live demonstration of diagnostic ability.

**The upgrade path.** A `case_studies.is_illustrative` flag renders the badge
automatically. When a client approves, an editor flips the flag, adds the client
name and logo, and the badge disappears. It must be impossible to publish a
fabricated result as a real one — the database enforces what the ethics require.

---

## 5. Lead capture and the Core Four
...
Lead scoring inputs: the chosen problem area, whether the maturity score was
completed and what it said, company size if given, the length and specificity of
the free-text answer, whether the email is a work domain, and the UTM source. High
scores notify the founder immediately; low scores enter nurture.
...
The demo has roughly 3,100 words on one page. Not badly written — much of it is
unusually good — but arranged so that every section has equal weight. The fix is
...
**Beekal sounds like:** a senior operator who has seen the inside of a badly run
business and is not going to be diplomatic about it. Short sentences. Concrete
nouns. Numbers instead of adjectives. Comfortable saying "no" and "that is not
your problem".

**Six rules, applied to every string in the database:**
...
5. **Cut the sentence that only sets up the next one** (`Talk Less Sell More`).
   Most corporate opening sentences are throat-clearing.
6. **One idea per paragraph, one job per section.** If a section is doing two
   jobs, it is two sections or one of them is unnecessary.

### 6.3 Banned vocabulary

Enforced by a lint rule over content fields, warning-level, overridable with a
note — because the point is deliberateness, not censorship.

> leverage · synergy · cutting-edge · world-class · best-in-class · seamless ·
...
### 6.4 Headline formulas that fit this business
...
### 6.6 Nurture sequences

`$100M Lead Nurture` and `$100M Marketing Machine`. Every capture path gets a
sequence, all editable as templates in the admin so no email is trapped in code:

- **Score completed, PDF requested** — 4 emails over 12 days, one per weak
  dimension, ending on the Assessment
- **Resource downloaded** — 3 emails over 9 days, the checklist applied to a real
  scenario
- **Form submitted, not yet qualified** — founder reply within one working day,
  then a single follow-up at day 7 and nothing further
- **Assessment delivered** — the roadmap's first item, revisited at 30 and 90 days

Consent is explicit and per-purpose, with a real unsubscribe. The demo's privacy
copy already promises no mailing list — so the marketing consent must be a
**separate, unticked** checkbox from the reply consent, and the sequences only run
for people who ticked it. That promise is now a schema constraint, not a policy.
